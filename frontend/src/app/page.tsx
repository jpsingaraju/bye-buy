...
function MagneticButton({ children, href }: { children: React.ReactNode; href: string }) {
  ...
  return (
    <motion.div onMouseMove={handleMouse} onMouseLeave={() => { x.set(0); y.set(0); }} className="inline-block">
      <motion.div style={{ x: springX, y: springY }}>
        <Link ref={ref} href={href} className="inline-flex items-center px-10 py-4 text-lg font-bold bg-primary text-white neo-border neo-shadow neo-hover">
          {children}
        </Link>
      </motion.div>
    </motion.div>
...
function CrossPostOrbit() {
  ...
  return (
    <svg viewBox={`0 0 ${VB} ${VB}`} className="w-[336px] h-[336px] mx-auto overflow-visible">
      {/* Orbit rings */}
      <circle cx={CX} cy={CY} r={R} fill="none" stroke="#FF5484" strokeWidth="1.5"
        strokeDasharray="6 6" opacity="0.25">
        <animateTransform attributeName="transform" type="rotate"
          from={`0 ${CX} ${CY}`} to={`360 ${CX} ${CY}`} dur="30s" repeatCount="indefinite" />
      </circle>
      <circle cx={CX} cy={CY} r={R - 14} fill="none" stroke="#5B4CFF" strokeWidth="1"
      ...
      {/* Center AI node */}
      <rect x={CX - 32} y={CY - 32} width="64" height="64" fill="#5B4CFF" stroke="#1A1A2E" strokeWidth="2.5">
        <animate attributeName="opacity" values="1;0.8;1" dur="2s" repeatCount="indefinite" />
      </rect>
      <text x={CX} y={CY + 8} textAnchor="middle" fill="white" fontSize="22" fontWeight="800"
        fontFamily="var(--font-space-grotesk), sans-serif">AI</text>
        ...
        return (
          <g key={name}>
            <line x1={CX} y1={CY} x2={px} y2={py}
              stroke="#FF5484" strokeWidth="1.5" strokeDasharray="4 4" opacity="0.2" />
            ...
            >
              <rect x={px - 38} y={py - 14} width="76" height="28" rx="0"
                fill="white" stroke="#1A1A2E" strokeWidth="2" />
              <text x={px} y={py + 5} textAnchor="middle" fill="#1A1A2E"
                fontSize="12" fontWeight="700" fontFamily="var(--font-space-grotesk), sans-serif">
                {name}
              </text>
            </motion.g>
            ...
            <motion.g
              initial={{ opacity: 0, scale: 0 }}
              animate={{ opacity: 1, scale: 1 }}
              transition={{ delay: 1.5 + i * 0.2, type: "spring", stiffness: 400, damping: 12 }}
            >
              <circle cx={px + 32} cy={py - 10} r="8" fill="#FF5484" stroke="#1A1A2E" strokeWidth="1.5" />
              <text x={px + 32} y={py - 6} textAnchor="middle" fill="white" fontSize="10" fontWeight="800">✓</text>
            </motion.g>
          </g>
        );
      })}
    ...
    <div className="relative w-full h-full min-h-[520px] flex flex-col items-center">
      ...
      <div className="relative flex-1 w-full flex items-center justify-center">
        ...
        <AnimatePresence mode="wait">
          {/* Step 0: Sell it */}
          {step === 0 && (
            <motion.div key="s0" initial={enter} animate={visible} exit={exit} transition={springy}
              className="flex flex-col items-center gap-3 z-10">
              <div className="w-[336px] h-[336px] bg-surface neo-border neo-shadow overflow-hidden flex flex-col">
                <div className="flex-1 bg-gradient-to-br from-primary/15 to-secondary/10 flex items-center justify-center border-b-2 border-ink">
                  <span className="text-6xl">📱</span>
                </div>
                <div className="p-4">
                  <p className="font-bold text-lg text-ink">iPhone 15 Pro</p>
                  <p className="text-sm text-primary/70 mt-0.5 font-medium">Like New · Seattle</p>
                  <p className="font-black text-2xl text-primary mt-1.5">$450</p>
                </div>
              </div>
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}
                className="flex items-center gap-2 px-4 py-2 bg-primary/10 neo-border">
                <span className="text-sm font-bold text-ink">Want to sell it?</span>
                <span className="text-sm font-black text-primary">We&apos;ll handle everything.</span>
              </motion.div>
            </motion.div>
          )}
          ...
          {step === 1 && (
            <motion.div key="s1" initial={enter} animate={visible} exit={exit} transition={springy}
              className="flex flex-col items-center z-10">
              <CrossPostOrbit />
              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}
                className="text-xs font-bold text-ink -mt-2">
                Posted to <span className="text-primary">10+ platforms</span> simultaneously
              </motion.p>
            </motion.div>
          )}
          ...
          {/* Step 2: Negotiate */}
          {step === 2 && (
            <motion.div key="s2" initial={enter} animate={visible} exit={exit} transition={springy}
              className="flex flex-col items-center z-10">
              <div className="w-[336px] h-[336px] bg-surface neo-border neo-shadow overflow-hidden flex flex-col">
                <div className="px-4 py-2.5 bg-primary/10 border-b-2 border-ink flex items-center gap-2 shrink-0">
                  ...
                  <span className="text-sm font-bold text-ink">Buyer · Facebook</span>
                </div>
                <div className="p-3 space-y-2 flex-1 overflow-hidden">
                  {[
                    { from: "buyer", text: "Is this still available?", delay: 0.15 },
                    { from: "agent", text: "Yes! It's in great condition.", delay: 0.5 },
                    { from: "buyer", text: "Would you take $400?", delay: 1.0 },
                    { from: "agent", text: "I can do $420, lowest I'll go.", delay: 1.5 },
                    ...
                    <motion.div key={i}
                      ...
                      <div className={`px-3 py-1.5 text-sm font-medium max-w-[85%] border-2 border-ink ${
                        msg.from === "agent" ? "bg-secondary/10" : "bg-primary/5"
                      }`}>
                        {msg.from === "agent" && (
                          <span className="text-secondary font-bold text-[10px] block mb-0.5">AI Agent</span>
                        )}
          ...
          {/* Step 3: Deal */}
          {step === 3 && (
            <motion.div key="s3" initial={enter} animate={visible} exit={exit} transition={springy}
              className="flex flex-col items-center gap-3 z-10">
              <motion.div initial={{ scale: 0.5, rotate: -8 }} animate={{ scale: 1, rotate: 0 }}
                ...
                <p className="text-sm text-primary font-medium mt-1">iPhone 15 Pro</p>
                <div className="mt-4 flex items-center justify-center gap-3">
                  <span className="line-through text-ink/50 font-bold text-lg">$450</span>
          ...
          {/* Step 4: Paid */}
          {step === 4 && (
            <motion.div key="s4" initial={enter} animate={visible} exit={exit} transition={springy}
              className="flex flex-col items-center gap-3 z-10">
              <motion.div initial={{ scale: 0.7 }} animate={{ scale: 1 }}
                ...
                <p className="text-sm text-ink font-bold mt-1.5">Deposited to your account</p>
              </motion.div>
              <motion.div initial={{ width: 0 }} animate={{ width: "21rem" }}
    ...
    <div className="min-h-screen flex flex-col relative overflow-hidden">
      ...
      <header className="relative z-10 px-6 sm:px-10 lg:px-16 py-6">
        <div className="flex items-center gap-2.5">
          <Image src="/logo.png" alt="bye! buy!" width={44} height={44} className="w-11 h-11" />
